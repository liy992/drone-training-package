# Isaac Sim + Pegasus + PX4 无人机基础控制与 EGO 避障

在 Isaac Sim 中加载无人机，并通过 QGC、ROS 2 或 EGO-Planner 进行控制与避障演示。

Pegasus 负责将仿真中的无人机接入 PX4。PX4 接收 QGroundControl 或 ROS 2 的控制命令，并驱动无人机运动。

典型验证流程为：使用官方 Iris 验证基础链路，使用 ROS 2 控制无人机，替换为目标无人机资产，再接入雷达点云和 EGO-Planner 避障。
...
Isaac Sim 场景
  ...
  -> 替换为自定义目标无人机
  -> 单独加载和控制货舱资产
  -> RTX LiDAR 发布点云、TF、Odom
  -> EGO-Planner 输出轨迹并接 PX4 Offboard
```
...
| 模块六：雷达点云与 EGO-Planner 避障 | LiDAR、点云、TF、Odom、EGO 输入输出 | Isaac 发布感知话题，EGO 输出 `/position_cmd` |
...
| 5 | 雷达点云 + TF/Odom + EGO-Planner | 运行 `run_ego_stage_04.sh`、`run_ego_stage_05.sh`、`run_ego_stage_06.sh` 完成避障链路验证 |

运行异常时可查看 `docs/07_疑难杂症_QA.md`。该文件为问题排查文档，按报错现象整理检查顺序。
