...
```text
/root/PegasusSimulator/extensions/pegasus.simulator/pegasus/simulator/assets/Robots/
```
...
如果只用脚本加载，不需要出现在 Pegasus UI 中，也可以把 USD 放在自己的 `assets/robots/` 下，然后在脚本里传入绝对路径或相对路径。
...
![Pegasus UI 中出现比赛无人机资产](assets/isaac_px4_training/pegasus_custom_drone_ui.png)
