...
- 说明无人机仿真需要飞控闭环的原因。
- 明确 Isaac Sim、Pegasus、PX4、QGC、ROS 2 的职责边界。
- 理解 PX4 的几个基础概念：local position、模式切换、arm、持续 setpoint。

前置条件：

- 使用 Ubuntu 20.04 + ROS 2 Foxy、Ubuntu 22.04 + ROS 2 Humble 或 Ubuntu 24.04 + ROS 2 Jazzy。
- 验证环境为 Ubuntu 24.04.4，并安装 ROS 2 Jazzy。
- Isaac Sim 可以正常启动。
...
整体关系为：QGC 查看飞控状态，ROS 2 发送控制目标，Pegasus 将 PX4 的执行器输出作用到 Isaac Sim 中的无人机。
...
QGroundControl 通常简称 QGC，是无人机地面站。它通过 MAVLink 连接 PX4。调试过程中可保持 QGC 打开，以便查看飞控状态和告警信息。
...
| 绑定已有无人机 prim | USD 场景里已经有 `/World/quadrotor` | 脚本调用 `Multirotor(..., attach_existing=True)` 绑定已有 prim。该能力需要 Pegasus 版本或定制修改支持 |
...
| 模块 | 作用 | 简要记忆 |
| --- | --- | --- |
| Commander | 管理解锁、上锁、飞行模式、安全状态 | 无人机不能飞时，优先查看 preflight check |
...
| 控制方式 | 适合做什么 | 不适合做什么 |
| --- | --- | --- |
| 直接移动模型 | 检查 USD 资产、起始位置、传感器位置 | 不能说明飞控链路正常，也不能验证真实飞行控制 |
| QGC 手动控制 | 检查 PX4 是否 Ready To Fly，验证 arm、takeoff、land | 不适合自动任务和程序化路径控制 |
...
需注意持续发布要求。PX4 不接受只发送一次 setpoint 的外部控制。setpoint 或 Offboard 控制模式停止后，PX4 会认为外部链路中断，并退出 Offboard 或触发 failsafe。
